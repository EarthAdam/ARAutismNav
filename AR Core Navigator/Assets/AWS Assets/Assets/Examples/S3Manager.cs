...
namespace AWSSDK.Examples
{
    public class S3Manager : MonoBehaviour
    {
        ...
        [SerializeField] private string cognitoIdentityRegion = RegionEndpoint.USEast2.SystemName;
        [SerializeField] private string s3Region = RegionEndpoint.USEast2.SystemName;

        // Variables privates
        private int timeoutGetObject = 60; // seconds
        ...
        public void GetObjectBucket(string S3BucketName, string fileNameOnBucket, Action<GetObjectResponse, string> result)
        {
            ...
            */
            ...
            Client.GetObjectAsync(S3BucketName, fileNameOnBucket, (responseObj) =>
            {
                if (responseObj.Response.ResponseStream != null)
                {
                    using (var fs = System.IO.File.Create(@Application.persistentDataPath+@"/test.txt"))
                    {
